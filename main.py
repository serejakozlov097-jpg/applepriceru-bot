...
def get_category_menu():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=cat, callback_data=f"category_{cat}")] for cat in PRODUCTS.keys()])

def get_model_menu(category):
    models = PRODUCTS[category].keys()
    buttons = [[InlineKeyboardButton(text=model, callback_data=f"model_{category}_{model}")] for model in models]
    buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_categories")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def get_memory_menu(category, model):
    memories = PRODUCTS[category][model].keys()
    buttons = [[InlineKeyboardButton(text=mem, callback_data=f"memory_{category}_{model}_{mem}")] for mem in memories]
    buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"back_to_models_{category}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def get_color_menu(category, model, memory):
    colors = PRODUCTS[category][model][memory].keys()
    buttons = [[InlineKeyboardButton(text=color, callback_data=f"color_{category}_{model}_{memory}_{color}")] for color in colors]
    buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"back_to_memory_{category}_{model}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def get_market_menu(category, model, memory, color):
    marketplaces = PRODUCTS[category][model][memory][color]
    buttons = [[InlineKeyboardButton(text=market, url=url)] for market, url in marketplaces.items()]
    buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"back_to_colors_{category}_{model}_{memory}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

# ===== Обработчики =====
@dp.message(Command("start"))
async def start(message: Message):
    await message.answer("Привет! Выберите категорию:", reply_markup=get_category_menu())

@dp.callback_query(F.data.startswith("category_"))
async def category_handler(callback: CallbackQuery):
    category = callback.data.split("_", 1)[1]
    await callback.message.edit_text(f"Выберите модель в категории {category}:", reply_markup=get_model_menu(category))

@dp.callback_query(F.data.startswith("model_"))
async def model_handler(callback: CallbackQuery):
    _, category, model = callback.data.split("_", 2)
    await callback.message.edit_text(f"Выберите объём памяти для {model}:", reply_markup=get_memory_menu(category, model))

@dp.callback_query(F.data.startswith("memory_"))
async def memory_handler(callback: CallbackQuery):
    _, category, model, memory = callback.data.split("_", 3)
    await callback.message.edit_text(f"Выберите цвет для {model} ({memory}):", reply_markup=get_color_menu(category, model, memory))

@dp.callback_query(F.data.startswith("color_"))
async def color_handler(callback: CallbackQuery):
    _, category, model, memory, color = callback.data.split("_", 4)
    await callback.message.edit_text(f"Выберите маркетплейс для {model} {memory} {color}:", reply_markup=get_market_menu(category, model, memory, color))

# ===== Кнопки "Назад" =====
@dp.callback_query(F.data.startswith("back_to_categories"))
...
async def back_to_models(callback: CallbackQuery):
    category = callback.data.replace("back_to_models_", "")
    await callback.message.edit_text(f"Выберите модель в категории {category}:", reply_markup=get_model_menu(category))
